...
/datum/controller/subsystem/ticker/proc/declare_completion()
	...
	for (var/mob/living/silicon/ai/aiPlayer in mob_list)
		if (aiPlayer.stat != 2)
			to_world("<span class='filter_system'><b>[aiPlayer.name] законы в конце раунда были:</b></span>") // VOREStation edit
		else
			to_world("<span class='filter_system'><b>[aiPlayer.name] законы, когда произошла деактивация:</b></span>") // VOREStation edit
		aiPlayer.show_laws(1)

		if (aiPlayer.connected_robots.len)
			var/robolist = "<b>Верными приспешниками ИИ были:</b> "
			for(var/mob/living/silicon/robot/robo in aiPlayer.connected_robots)
				robolist += "[robo.name][robo.stat?" (Деактивирован), ":", "]"  // VOREStation edit
