...
datum/objective/anti_revolution/brig
	...
	find_target()
		..()
		if(target && target.current)
			explanation_text = "Поместите [target.current.real_name] [target.assigned_role] в тюрьму, на 20 минут, чтобы подать пример."
		else
			explanation_text = "Свободная цель"
		return target
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Поместите [target.current.real_name] [!role_type ? target.assigned_role : target.special_role] в тюрьму, на 20 минут, чтобы подать пример."
		else
			explanation_text = "Свободная цель"
		return target
...
/datum/objective/cult/eldergod
	explanation_text = "Призвать Нар-Си с помощью соответствующей руны (Ад Присоединиться Я). Это сработает, только если девять культистов будут стоять на нем и вокруг него. Руна обращения - (Ад Кровь Присоединение)."
...
/datum/objective/cult/sacrifice/find_target()
	...
	if(possible_targets.len > 0)
		target = pick(possible_targets)
	if(target) explanation_text = "Принесите в жертву [target.name], [target.assigned_role]. Для этого вам понадобится руна жертвоприношения (адская кровь) и три послушника."
