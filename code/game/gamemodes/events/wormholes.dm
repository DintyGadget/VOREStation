/proc/wormhole_event(var/set_duration = 5 MINUTES, var/wormhole_duration_modifier = 1)
	spawn()
		...
		if(pick_turfs.len)
			...
			command_announcement.Announce("На станции обнаружены пространственно-временные аномалии. Нет дополнительных данных.", "Anomaly Alert", new_sound = 'sound/AI/spanomalies.ogg')
