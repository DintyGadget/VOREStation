...
/datum/category_item/player_setup_item/general/body/content(var/mob/user)
	...
	var/datum/species/mob_species = GLOB.all_species[pref.species]
	. += "<table><tr style='vertical-align:top'><td><b>Рандомизатор</b> "
	...
	. += "Органы: <a href='?src=\ref[src];organs=1'>Выбрать</a><br>"
	...
	. += "<h2>Параметры генетики</h2>"

	var/list/ear_styles = pref.get_available_styles(global.ear_styles_list)
	var/datum/sprite_accessory/ears/ear = ear_styles[pref.ear_style]
	. += "<b>Уши</b><br>"
	if(istype(ear))
		. += " Стиль: <a href='?src=\ref[src];ear_style=1'>[ear.name]</a><br>"
		if(ear.do_colouration)
			. += "<a href='?src=\ref[src];ear_color=1'>Изм. цвет</a> [color_square(pref.r_ears, pref.g_ears, pref.b_ears)]<br>"
		if(ear.extra_overlay)
			. += "<a href='?src=\ref[src];ear_color2=1'>Изм. втор. цвет</a> [color_square(pref.r_ears2, pref.g_ears2, pref.b_ears2)]<br>"
		if(ear.extra_overlay2)
			. += "<a href='?src=\ref[src];ear_color3=1'>Изм. трет. цвет</a> [color_square(pref.r_ears3, pref.g_ears3, pref.b_ears3)]<br>"
	else
		. += " Стиль: <a href='?src=\ref[src];ear_style=1'>Выбрать</a><br>"

	var/list/tail_styles = pref.get_available_styles(global.tail_styles_list)
	var/datum/sprite_accessory/tail/tail = tail_styles[pref.tail_style]
	. += "<b>Хвост</b><br>"
	if(istype(tail))
		. += " Стиль: <a href='?src=\ref[src];tail_style=1'>[tail.name]</a><br>"
		if(tail.do_colouration)
			. += "<a href='?src=\ref[src];tail_color=1'>Изм. цвет</a> [color_square(pref.r_tail, pref.g_tail, pref.b_tail)]<br>"
		if(tail.extra_overlay)
			. += "<a href='?src=\ref[src];tail_color2=1'>Изм. втор. цвет</a> [color_square(pref.r_tail2, pref.g_tail2, pref.b_tail2)]<br>"
		if(tail.extra_overlay2)
			. += "<a href='?src=\ref[src];tail_color3=1'>Изм. трет. цвет</a> [color_square(pref.r_tail3, pref.g_tail3, pref.b_tail3)]<br>"
	else
		. += " Стиль: <a href='?src=\ref[src];tail_style=1'>Выбрать</a><br>"

	var/list/wing_styles = pref.get_available_styles(global.wing_styles_list)
	var/datum/sprite_accessory/wing/wings = wing_styles[pref.wing_style]
	. += "<b>Крылья</b><br>"
	if(istype(wings))
		. += " Стиль: <a href='?src=\ref[src];wing_style=1'>[wings.name]</a><br>"
		if(wings.do_colouration)
			. += "<a href='?src=\ref[src];wing_color=1'>Изм. цвет</a> [color_square(pref.r_wing, pref.g_wing, pref.b_wing)]<br>"
		if(wings.extra_overlay)
			. += "<a href='?src=\ref[src];wing_color2=1'>Изм. втор. цвет</a> [color_square(pref.r_wing2, pref.g_wing2, pref.b_wing2)]<br>"
		if(wings.extra_overlay2)
			. += "<a href='?src=\ref[src];wing_color3=1'>Изм. трет. цвет</a> [color_square(pref.r_wing3, pref.g_wing3, pref.b_wing3)]<br>"
	else
		. += " Стиль: <a href='?src=\ref[src];wing_style=1'>Выбрать</a><br>"
	...
	if(href_list["random"])
		...
		if(mob_species.descriptors)
			...
			if(pref.body_descriptors[desc_id])
				...
				var/choice = input("Пожалуйста, выберите дескриптор.", "Descriptor") as null|anything in descriptor.chargen_value_descriptors
		...
		*/ //VOREStation Removal End
		...
		var/new_ear_style = input(user, "Выберите уши:", "Изменение Персонажа", pref.ear_style) as null|anything in pref.get_available_styles(global.ear_styles_list)
		if(new_ear_style)
		...
		var/new_earc = input(user, "Выберите цвет ушей:", "Изменение Персонажа",
		...
		var/new_earc2 = input(user, "Выберите вторичный цвет ушей:", "Изменение Персонажа",
		...
		var/new_earc3 = input(user, "Выберите третичный цвет ушей:", "Изменение Персонажа",
		...
		var/new_tail_style = input(user, "Выберите хвост:", "Изменение Персонажа", pref.tail_style) as null|anything in pref.get_available_styles(global.tail_styles_list)
		if(new_tail_style)
			pref.tail_style = new_tail_style
		return TOPIC_REFRESH_UPDATE_PREVIEW

	else if(href_list["tail_color"])
		var/new_tailc = input(user, "Выберите цвет хвоста/тавра вашего персонажа:", "Изменение Персонажа",
		...
		var/new_tailc2 = input(user, "Выберите вторичный цвет хвоста/тавра вашего персонажа:", "Изменение Персонажа",
		...
		var/new_tailc3 = input(user, "Выберите третичный цвет хвоста/тавра вашего персонажа:", "Изменение Персонажа",
		...
		var/new_wing_style = input(user, "Выберите стиль крыльев:", "Изменение Персонажа", pref.wing_style) as null|anything in pref.get_available_styles(global.wing_styles_list)
		if(new_wing_style)
		...
		var/new_wingc = input(user, "Choose your character's wing colour:", "Изменение Персонажа",
		...
		var/new_wingc2 = input(user, "Choose your character's secondary wing colour:", "Изменение Персонажа",
...
/datum/category_item/player_setup_item/general/body/proc/SetSpecies(mob/user)
	...
	if(current_species.spawn_flags & SPECIES_CAN_JOIN)
		switch(current_species.rarity_value)
			...
			if(5)
				...
				dat += "</br><b>Может присутствовать на человеческих станциях.</b>"
	...
	if(current_species.flags & NO_DEFIB)
		dat += "</br><b>Не подлежит дефибрилляции.</b>"
