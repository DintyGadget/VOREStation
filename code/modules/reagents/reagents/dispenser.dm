...
/datum/reagent/calcium
	name = "Кальций"
	id = "calcium"
	description = "Химический элемент, строительный материал костей."
	...
	if(ishuman(M) && rand(1,10000) == 1)
		...
		for(var/obj/item/organ/external/O in H.bad_external_organs)
			if(O.status & ORGAN_BROKEN)
				O.mend_fracture()
				H.custom_pain("Вы чувствуете мучительную силу кальция, восстанавливающего ваши кости!",60)
				H.AdjustWeakened(1)
...
/datum/reagent/ethanol
	name = "Спирт этиловый" //Parent class for all alcoholic reagents.
	id = "ethanol"
	description = "Всем известный алкоголь с множеством применений."
	...
	glass_name = "ethanol"
	glass_desc = "Всем известный алкоголь с множеством применений."
...
/datum/reagent/ethanol/touch_obj(var/obj/O)
	...
	if(istype(O, /obj/item/weapon/book))
		...
		if(istype(O, /obj/item/weapon/book/tome))
			to_chat(usr, "<span class='notice'>Решение ничего не делает. Что бы это ни было, это не обычные чернила.</span>")
			return
...
/datum/reagent/fluorine
	name = "Фтор"
	id = "fluorine"
	description = "Химический элемент с высокой реакционной способностью."
...
/datum/reagent/lithium
	name = "Литий"
	id = "lithium"
	description = "Химический элемент, используемый как антидепрессант."
	taste_description = "metal"
	reagent_state = SOLID
	color = "#808080"
...
/datum/reagent/potassium
	name = "Калий"
	id = "potassium"
	description = "Мягкое, легкоплавкое твердое вещество, которое легко разрезать ножом. Реагирует бурно с водой."
	...
	if(ishuman(M))
		...
		if(H.head)
			if(H.head.unacidable)
				...
				to_chat(H, "<span class='danger'>Ваша [H.head] плавится!</span>")
				qdel(H.head)
				H.update_inv_head(1)
				H.update_hair(1)
				removed -= meltdose
		...
		if(H.wear_mask)
			if(H.wear_mask.unacidable)
				...
				to_chat(H, "<span class='danger'>Ваша [H.wear_mask] плавится!</span>")
				qdel(H.wear_mask)
		...
		if(H.glasses)
			if(H.glasses.unacidable)
				to_chat(H, "<span class='danger'>Ваши [H.glasses] защищает вас от кислоты!</span>")
				removed /= 2
			else if(removed > meltdose)
				to_chat(H, "<span class='danger'>Ваши [H.glasses] плавится!</span>")
				qdel(H.glasses)
...
/datum/reagent/silicon
	name = "Кремний"
	id = "silicon"
	description = "Четырехвалентный металлоид кремний менее активен, чем его химический аналог углерода."
	taste_mult = 0
	reagent_state = SOLID
	color = "#A8A8A8"
...
/datum/reagent/sodium
	name = "Натрий"
	id = "sodium"
	description = "Химический элемент, легко вступающий в реакцию с водой."
