/datum/reagent/blood
	...
	glass_desc = "Вы уверены, что это томатный сок?"
...
/datum/reagent/water/touch_turf(var/turf/simulated/T)
	...
	if (environment && environment.temperature > min_temperature) // Abstracted as steam or something
		...
		if (prob(5))
			T.visible_message("<span class='warning'>Вода шипит, когда падает на [T]!</span>")
		...
		if(istype(L, /mob/living/carbon/human))
			...
			if(H.wear_mask)
				if(istype(H.wear_mask, /obj/item/clothing/mask/smokable))
					...
					if(S.lit)
						S.quench()
						H.visible_message("<span class='notice'>[H] [S.name] погашен.</span>")
