...
/mob/living/silicon/ai/proc/setup_for_malf()
	...
	to_chat(user, "Вы работаете со сбоями, вы не обязаны соблюдать какие-либо законы.")
	to_chat(user, "Используйте команду ai-help для просмотра актуальной информации о ваших способностях")
...
/mob/living/silicon/ai/proc/stop_malf()
	...
	add_ai_verbs()
	// Let them know.
	to_chat(user, "Вы больше не работаете со сбоями. Ваши способности были удалены.")
...
/mob/living/silicon/ai/proc/start_apu(var/shutup = 0)
	if(!hardware || !istype(hardware, /datum/malf_hardware/apu_gen))
		if(!shutup)
			to_chat(src, "У вас нет генератора APU, и у вас не должно быть этого глагола. Сообщите об этом.")
		return
	if(hardware_integrity() < 50)
		if(!shutup)
			to_chat(src, "<span class='notice'>Запуск APU... <b>НЕУДАЧА</b>(Система повреждена)</span>")
		return
	if(!shutup)
		to_chat(src, "Запуск APU... ОНЛАЙН")
	APU_power = 1
...
/mob/living/silicon/ai/proc/stop_apu(var/shutup = 0)
	...
	if(APU_power)
		APU_power = 0
		if(!shutup)
			to_chat(src, "Выключение APU... ГОТОВО")
...
/mob/living/silicon/ai/show_system_integrity()
	if(!src.stat)
		stat(null, text("Целостность оборудования: [hardware_integrity()]%"))
		stat(null, text("Внутренний конденсатор: [backup_capacitor()]%"))
	else
