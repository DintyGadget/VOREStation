...
/obj/item/weapon/grab/New(mob/user, mob/victim)
	...
	affecting.reveal("<span class='warning'>Вы раскрываетесь, когда [assailant] хватает вас.</span>")
	assailant.reveal("<span class='warning'>Вы раскрываете себя, когда хватаете [affecting].</span>")
...
/obj/item/weapon/grab/proc/handle_eye_mouth_covering(mob/living/carbon/target, mob/user, var/target_zone)
	...
	switch(target_zone)
		if(O_MOUTH)
			if(announce)
				user.visible_message("<span class='warning'>[user] прикрывает рот [target]!</span>")
			if(target.silent < 3)
				target.silent = 3
		if(O_EYES)
			if(announce)
				assailant.visible_message("<span class='warning'>[assailant] прикрывает глаза [affecting]!</span>")
		...
		if(BP_HEAD)
			if(force_down)
				if(announce)
					assailant.visible_message("<span class='warning'>[assailant] садится на лицо [target]!</span>")
...
/obj/item/weapon/grab/proc/s_click(obj/screen/S)
	...
	if(state < GRAB_AGGRESSIVE)
		...
		if(!affecting.lying || size_difference(affecting, assailant) > 0)
			assailant.visible_message("<span class='warning'>[assailant] хватает [affecting] агрессивной (руки)!</span>")
		else
			assailant.visible_message("<span class='warning'>[assailant] прижимает [affecting] к земле (руки)!</span>")
		...
		assailant.visible_message("<span class='warning'>[assailant] усиливает хватку на [affecting] (шея)!</span>")
		state = GRAB_NECK
		...
		assailant.visible_message("<span class='danger'>[assailant] начинает усиливать хватку на шее [affecting]!</span>")
	...
	if(M == affecting)
		if(ishuman(affecting))
			...
			switch(assailant.a_intent)
				if(I_HELP)
					if(force_down)
						to_chat(assailant, "<span class='warning'>Вы больше не прижимаете [affecting] к земле.</span>")
						force_down = 0
						return
					if(state >= GRAB_AGGRESSIVE)
...
/obj/item/weapon/grab/proc/reset_kill_state()
	if(state == GRAB_KILL)
		...
		assailant.visible_message("<span class='warning'>[assailant] теряет хватку на шее [affecting]!</span>")
		hud.icon_state = "kill"
		state = GRAB_NECK
...
/obj/item/weapon/grab/proc/handle_resist()
	...
	if(prob(break_chance))
		if(state == GRAB_KILL)
			...
			affecting.visible_message("<span class='warning'>[affecting] вырывается из захвата [assailant] [grab_name]!</span>")
		qdel(src)
